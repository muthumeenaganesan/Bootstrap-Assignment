...
var metrics=(function(){var h=null;
var z={pages:{},clicks:{},variables:{}};
var b={dl:(function(){var C=null;
var D=null;
var E=null;
return{initialize:function(){if(!window.digitaldata){window.digitaldata={}
}},reset:function(F){if(window.digitaldata&&F){window.digitaldata={};
C=null;
if(window.console&&window.console.log){console.log("metrics > hard reset")
}}else{if(window.digitaldata&&C){window.digitaldata=jQuery.extend(true,{},C);
if(window.console&&window.console.log){console.log("metrics > reset");
console.log(window.digitaldata);
console.log("metrics > reset > initialState");
console.log(C)
}}}},saveTemporaryState:function(){if(!D){D=jQuery.extend(true,{},window.digitaldata);
E=jQuery.extend(true,{},C)
}},restoreTemporaryState:function(){if(D&&E){window.digitaldata=jQuery.extend(true,{},D);
C=jQuery.extend(true,{},E);
D=null;
E=null
}},resetTemporaryState:function(){if(D&&E){D=null;
E=null
}},setVariable:function(F,M,J,K,I){if(!window.digitaldata){window.digitaldata={}
}var G=null;
var L=null;
if(J&&K&&I&&I.variables&&I.variables.length>0){for(var H=0;
H<I.variables.length;
H++){L=I.variables[H].variableName;
if(L){if(L===J){G=I.variables[H].fireOnce?I.variables[H].fireOnce:null;
break
}}}}var N=K+J+M;
if(!G||(G&&!z.variables[N])){B(F,M);
window.digitaldata=r(window.digitaldata,F);
if(window.console&&window.console.log){console.log("metrics > setVariable >");
console.log(F)
}if(G&&N){z.variables[N]=true
}}},setEvent:function(F,K,I){if(!window.digitaldata){window.digitaldata={}
}var G=null;
var J="event";
var L=null;
var M=F&&F.event&&F.event.action?F.event.action:null;
if(K&&I&&I.variables&&I.variables.length>0){for(var H=0;
H<I.variables.length;
H++){L=I.variables[H].variableName;
if(L){if(L===J){G=I.variables[H].fireOnce?I.variables[H].fireOnce:null;
break
}}}}var N=K+J+M;
if(!G||(G&&!z.variables[N])){window.digitaldata=r(window.digitaldata,F);
if(window.console&&window.console.log){console.log("metrics > setEvent >");
console.log(F)
}if(G&&N){z.variables[N]=true
}}},pageEvent:function(H,K,J,I){var F=null;
if(!H&&K&&Array.isArray(K)&&K.length>0){H=K[0]
}if(H){var L=window.digitaldata?JSON.stringify(window.digitaldata)+H:"";
if(!z.pages[L]){if(window._satellite&&window._satellite.track){_satellite.track(H);
if(window.console&&window.console.log){console.log("metrics > pageEvent > "+H)
}if(L){z.pages[L]=true
}}}}if(C===null){C=jQuery.extend(true,{},window.digitaldata);
if(I&&I.variables&&I.variables.length>0){F=jQuery.extend(true,{},C);
for(var G=0;
G<I.variables.length;
G++){if(I.variables[G]&&I.variables[G].excludeFromInitialState&&I.variables[G].variableName){f(F,I.variables[G].variableName)
}}C=jQuery.extend(true,{},F)
}console.log("metrics > pageEvent > initialState");
console.log(C)
}},clickEvent:function(I,L,K,J){var H=null;
if(!I&&L&&Array.isArray(L)&&L.length>0){I=L[0]
}if(I){H=K&&K.fireOnce?K.fireOnce:null;
var F=window.digitaldata?JSON.stringify(window.digitaldata)+I:"";
if((!H)||(H&&!z.clicks[F])){if(window.console&&window.console.log){console.log("metrics > clickEvent > "+I)
}if(window._satellite&&window._satellite.track){_satellite.track(I);
if(H&&F){z.clicks[F]=true
}}}}if(K&&K.asPage){if(C===null){C=jQuery.extend(true,{},window.digitaldata);
if(J&&J.variables&&J.variables.length>0){initialStateCopy=jQuery.extend(true,{},C);
for(var G=0;
G<J.variables.length;
G++){if(J.variables[G]&&J.variables[G].excludeFromInitialState&&J.variables[G].variableName){f(initialStateCopy,J.variables[G].variableName)
}}C=jQuery.extend(true,{},initialStateCopy)
}console.log("metrics > clickEvent(asPage) > initialState");
console.log(C)
}}}}
})()};
function B(E,D){if(typeof E==="object"){for(var C in E){if(typeof E[C]==="object"){B(E[C],D)
}else{E[C]=D;
return
}}}}function f(D,C){Object.keys(D).forEach(function(E){if(E===C){delete D[C]
}else{if(typeof D[E]==="object"){f(D[E],C)
}}})
}function r(D,C){var F=Array.isArray(C);
var E=F&&[]||{};
if(F){D=D||[];
E=E.concat(D);
C.forEach(function(H,G){if(typeof E[G]==="undefined"){E[G]=H
}else{if(typeof H==="object"){E[G]=r(D[G],H)
}else{if(D.indexOf(H)===-1){E.push(H)
}}}})
}else{if(D&&typeof D==="object"){Object.keys(D).forEach(function(G){E[G]=D[G]
})
}Object.keys(C).forEach(function(G){if(typeof C[G]!=="object"||!C[G]){E[G]=C[G]
}else{if(!D[G]){E[G]=C[G]
}else{E[G]=r(D[G],C[G])
}}})
}return E
}function k(I,H,C){var F=h.variables[I],G=H?H[I]||"":"",E=C?C[I]||"":"";
if(E&&E.val){G=E.val
}else{if(F.val){if(typeof F.val==="function"){try{G=F.val(G,arguments.callee,H)
}catch(D){if(window.console&&window.console.log){console.log("metrics > getMetricsValue > error getting value for > id >"+I+" > "+D)
}}}else{if((G||"")!==""){G=G.val
}}}}if(G){try{G=G.replace(/({[a-zA-Z0-9_]*})/gm,function(J,K){return k(K,H,C)
})
}catch(D){if(window.console&&window.console.log){console.log("metrics > getMetricsValue > error getting dynamic value for > id >"+I+" > "+D)
}}}return G
}function w(O,D,N,G,J){for(var L in b){var M=D&&D.callOptions&&D.callOptions.asPage?D.callOptions.asPage:false;
var E=D&&D.callOptions&&D.callOptions.asOverlay?D.callOptions.asOverlay:false;
var K=D&&D.callOptions&&D.callOptions.inOverlay?D.callOptions.inOverlay:false;
if(E){b[L].saveTemporaryState();
b[L].reset(true)
}else{if(M||O==="pageEvent"){b[L].reset(true);
b[L].resetTemporaryState()
}else{if(!K){b[L].restoreTemporaryState()
}b[L].reset()
}}if(D.variables){Object.keys(D.variables).forEach(function(R){var Q=h.variables[R];
if(Q){var S=k(R,D.variables,G);
if(Q[L]){for(var P=0;
P<Q[L].length;
P++){b[L].setVariable(jQuery.extend(true,{},Q[L][P]),S,R,J,D.variableOptions?D.variableOptions:null)
}}}})
}if(D.events&&D.events.length>0){for(var I=0;
I<D.events.length;
I++){var C=D.events[I];
var F=h.events[C];
if(F&&F[L]){for(var H=0;
H<F[L].length;
H++){b[L].setEvent(F[L][H],J,D.variableOptions?D.variableOptions:null)
}}}}if(b[L][O]){b[L][O](N,D.metricsCallData&&D.metricsCallData[L]?D.metricsCallData[L]:null,D.callOptions?D.callOptions:null,D.variableOptions?D.variableOptions:null)
}}}function p(C){if(C&&document.cookie&&document.cookie!==""){var E=document.cookie.split(";");
for(var D=0;
D<E.length;
D++){var F=q(E[D]);
if(F.substr(0,C.length+1)===(C+"=")){return F.substr(C.length+1)
}}}return null
}function d(E,F,D){if(!D){D={}
}if(F===null){F="";
D.expires=-1
}var G=[E,"=",F];
if(D.expires){var C=new Date();
C.setTime(C.getTime()+(D.expires*24*60*60*1000));
G.push("; expires="+C.toUTCString())
}if(D.path){G.push("; path="+D.path)
}if(D.domain){G.push("; domain="+D.domain)
}if(D.secure){G.push("; secure")
}document.cookie=G.join("")
}function s(C){var D=p(C);
return(D)?decodeURIComponent(D):D
}function x(E,C,D){var I=(C)?p(E):s(E);
if(I){if(!D){D="&"
}I=I.split(D);
var G={};
for(var F=0;
F<I.length;
F++){var H=I[F].split("=");
G[H[0]]=(H.length<2)?null:decodeURIComponent(H[1])
}return G
}return null
}function e(E,G,C,D){var F=x(E,C,D);
return(F)?F[G]:F
}function m(D,E,C){if(!C){C={}
}d(D,(E)?encodeURIComponent(E):E,C)
}function A(E,F,D,H){var I=null;
if(F){var G=[];
for(var C in F){G.push(C+"="+encodeURIComponent(F[C]||""))
}I=G.join("&")
}if(H){m(E,I,D)
}else{d(E,I,D)
}}function g(D,G,F,C,H){var E=x(D)||{};
if(F===null){delete E[G];
if(l(E)){E=null
}}else{E[G]=F
}A(D,E,C,H)
}function l(C){for(var D in C){if(C.hasOwnProperty(D)){return false
}}return true
}function q(C){return C.replace(/^\s+|\s+$/g,"")
}function i(E,J,F){if(!E||!J||!F){return E
}var M=E.indexOf("?");
var G=E.indexOf("#");
var L=M!==-1;
var N=G!==-1;
var D="";
if(N&&M>G){return E
}if(E.indexOf(J)>-1){return E
}if(L){var K=E.substring(0,M+1);
var C=E.substring(M+1);
K+=J+"="+encodeURIComponent(F)+"&";
K+=C;
D=K
}else{if(N){var I=E.substring(0,G);
var H=E.substring(G);
I+="?"+J+"="+encodeURIComponent(F);
I+=H;
D=I
}else{D=E+"?"+J+"="+encodeURIComponent(F)
}}return D
}function n(){var C=jQuery(window).width();
if(C<768){return true
}return false
}function v(){var C=jQuery(window).width();
if(C>=768&&C<992){return true
}return false
}function y(){var C=jQuery(window).width();
if(C>=992){return true
}return false
}function j(){var C=jQuery("#metaLocale").attr("data-locale");
if(C&&C==="en_us"){return true
}return false
}function o(){var C=jQuery("#metaLocale").attr("data-locale");
if(C&&(C==="en_ca"||C==="fr_ca")){return true
}return false
}function u(D){var C=/(\w+):\/\/([\w@][\w.:@]+)\/?[\w\.?=%&=\-@\/$,]*/i;
var H;
var E;
var G=["intelliprice.com","www.intelliprice.com","shop.ford.com","www.shop.ford.com","ford.com","www.ford.com","showroom.ford.com","www.showroom.ford.com","inventory.ford.com","www.inventory.ford.com","m.ford.com","mqa.ford.com","wwwqa.ford.com","wwwqa.brandus.ford.com","wwwqa.brandauthorus.ford.com","www.brandus.ford.com","www.brandauthorus.ford.com"];
if((E=C.exec(D))!==null){if(E.length>=3){H=E[2]
}}if(D.charAt(0)==="/"){return true
}else{if(H){for(var F=0;
F<G.length;
F++){if(G[F]&&G[F].toLowerCase().indexOf(H.toLowerCase())>-1){return true
}}}}return false
}function a(D,F){var C=/(\w+):\/\/([\w@][\w.:@]+)\/?[\w\.?=%&=\-@\/$,]*/i;
var G;
var E;
if((E=C.exec(D))!==null){if(E.length>=3){G=E[2]
}}if(G&&F){if(G.toLowerCase().indexOf(F.toLowerCase())>-1){return true
}}return false
}function c(E,D){if(jQuery(this).attr("href")&&jQuery(this).attr("href").indexOf(E)===-1){D=D.toLocaleLowerCase();
var C=metrics.appendToQueryString(jQuery(this).attr("href"),E,D);
if(jQuery(this).attr("href")&&jQuery(this).attr("href").indexOf(E)===-1){jQuery(this).attr("href",C);
if(window.console&&window.console.log){window.console.log("appendCampaignCode > param > "+E+" > value > "+D)
}}}}var t={init:function(C){h=C;
for(var D in b){b[D].initialize()
}},pageEvent:function(D){var E=h.pages[D];
if(E){try{if((n()&&E.mobileOnly)||!E.mobileOnly){w("pageEvent",E,null,null,D)
}}catch(C){if(window.console&&window.console.log){window.console.log("metrics > pageEvent > an error occurred while firing a pageEvent > page > "+E+" > exception > "+C)
}}}},clickEvent:function(E,G,C){var F=h.clicks[E];
if(F){try{if((n()&&F.mobileOnly)||!F.mobileOnly){w("clickEvent",F,G,C,E)
}}catch(D){if(window.console&&window.console.log){window.console.log("metrics > pageEvent > an error occurred while firing a click event > click > "+F+" > exception > "+D)
}}}},getMetrics:k,getCookie:s,getCookieSub:e,appendCampaignCode:c,appendToQueryString:i,isUS:j,isCA:o,isInternalLink:u,domainContains:a};
return t
})();
var metricsConfig={};
metricsConfig.pages={homepage:{variables:{pageName:"{sitePrefix}:home",siteSection:"home",hierarchy:"home",userLanguage:"{userLanguage}",pageNameNoVehicle:"{sitePrefix}:home",radUIVersion:"{radUIVersion}",variantName:"home-load",contentType:"{contentModulePageName}:{contentModuleTemplate}:impress",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}"}},"fd results page":{variables:{pageName:"dd:find dealer:results",siteSection:"dealerships",hierarchy:"shopping tools:locate dealer",userLanguage:"{userLanguage}",pageNameNoVehicle:"dd:find dealer:results",tool:"event:find dealer",radUIVersion:"{radUIVersion}",variantName:"",onclick:"find dealer:current location",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}"},metricsCallData:{dl:["fd-results-load"]},events:["findDealerTool"],variableOptions:{variables:[{excludeFromInitialState:true,fireOnce:true,variableName:"tool"},{excludeFromInitialState:true,fireOnce:true,variableName:"event"}]}},"fd unaware":{variables:{pageName:"dd:find dealer:unaware",siteSection:"dealerships",hierarchy:"shopping tools:locate dealer",userLanguage:"{userLanguage}",pageNameNoVehicle:"dd:find dealer:unaware",radUIVersion:"{radUIVersion}",variantName:"fd-unaware",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}"}}};
metricsConfig.clicks={"global onclick":{variables:{onclickLinkName:"{customOnclickLinkName}",onclick:"{customOnclick}"},metricsCallData:{dl:["global-onclick"]}},"generic overlay search":{variables:{pageName:"{sitePrefix}:search overlay",siteSection:"search",hierarchy:"search",userLanguage:"{userLanguage}",pageNameNoVehicle:"{sitePrefix}:search overlay",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}",radUIVersion:"{radUIVersion}"},metricsCallData:{dl:["global-overlay"]}},"generic overlay home":{variables:{pageName:"{sitePrefix}:{customPageName}",siteSection:"home",hierarchy:"home",userLanguage:"{userLanguage}",pageNameNoVehicle:"{sitePrefix}:{customPageNameNoVehicle}",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}",radUIVersion:"{radUIVersion}"},metricsCallData:{dl:["global-overlay"]},callOptions:{asOverlay:true}},"share launch":{variables:{onclickLinkName:"{sitePrefix}:share:launch",onclick:"share:launch"},metricsCallData:{dl:["share-launch"]}},"share start":{variables:{onclickLinkName:"{sitePrefix}:share",onclick:"share start:{customShareSocialNetwork}:{customShareType}"},metricsCallData:{dl:["share-start"]}},"share complete":{variables:{onclickLinkName:"{sitePrefix}:share:complete",onclick:"share:{customShareSocialNetwork}:{customShareType}"},metricsCallData:{dl:["share-complete"]}},"save start":{variables:{onclickLinkName:"{sitePrefix}:save:start",onclick:"save:start",tool:"save:start:{customTool}"},metricsCallData:{dl:["save-start"]},events:["tool"]},"save complete":{variables:{onclickLinkName:"{sitePrefix}:save:complete",onclick:"save:complete",tool:"save:complete:{customTool}"},metricsCallData:{dl:["save-complete"]},events:["tool"]},"global click to chat":{variables:{onclickLinkName:"{sitePrefix}:click to chat",onclick:"click to chat"},metricsCallData:{dl:["click-to-chat"]}},"find dealer click to call":{variables:{onclickLinkName:"{sitePrefix}:find dealer:content:action",onclick:"call dealer",tool:"event:click to call"},metricsCallData:{dl:["global-fd-call"]},events:["clickToCall"]},"generic exit social":{variables:{onclickLinkName:"exit:social",onclick:"social:{customSocialNetwork}"},metricsCallData:{dl:["footer-exit"]}},"referral exit":{variables:{onclickLinkName:"referral:exit",onclick:"referral:{customOnclick}",referredTo:"{customReferredTo}"},metricsCallData:{dl:["footer-referral-exit"]},events:["brandReferral"]},"homepage onclick":{variables:{onclickLinkName:"{sitePrefix}:home:content:onpage",onclick:"hp content:{customClick}",contentType:"{contentModulePageName}:{contentModuleTemplate}:{customContentModuleOnPageClick}"},metricsCallData:{dl:["hp-onclick"]}},"homepage dealer onclick":{variables:{onclickLinkName:"{sitePrefix}:home:content:onpage",onclick:"hp content:call dealer",contentType:"{contentModulePageName}:{contentModuleTemplate}:find dealer"},metricsCallData:{dl:["hp-dealer-onclick"]}},"find dealer exit":{variables:{onclickLinkName:"referral:exit",onclick:"referral:dc:{customOnclick}",referredTo:"dc:{customReferredTo}"},metricsCallData:{dl:["fd-referral-exit"]},events:["brandReferral"]},"find dealer details":{variables:{pageName:"dd:find dealer:details",siteSection:"dealerships",hierarchy:"shopping tools:locate dealer",userLanguage:"{userLanguage}",pageNameNoVehicle:"dd:find dealer:details",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}",radUIVersion:"{radUIVersion}"},metricsCallData:{dl:["fd-details"]},callOptions:{asPage:true}},"find dealer onclick advanced search":{variables:{onclickLinkName:"dd:find dealer:results:advanced search",onclick:"find dealer:filter"},metricsCallData:{dl:["fd-onclick"]}},"find dealer onclick":{variables:{onclickLinkName:"dd:find dealer:details:cta",onclick:"find dealer details:{customOnclick}"},metricsCallData:{dl:["fd-onclick"]}},"find dealer call":{variables:{onclickLinkName:"dd:find dealer:details:cta",onclick:"find dealer details:{customOnclick}",tool:"event:click to call dealer"},metricsCallData:{dl:["fd-call"]},events:["clickToCallDealer"]},"find dealer call sales":{variables:{onclickLinkName:"dd:find dealer:details:cta",onclick:"find dealer details:{customOnclick}",tool:"call",toolDescriptor:"call sales"},metricsCallData:{dl:["fd-call-sales"]},events:["clickToCallSales"]},"fd results page":{variables:{pageName:"dd:find dealer:results",siteSection:"dealerships",hierarchy:"shopping tools:locate dealer",userLanguage:"{userLanguage}",pageNameNoVehicle:"dd:find dealer:results",tool:"event:find dealer",radUIVersion:"{radUIVersion}",variantName:"",onclick:"find dealer:{customOnclick}",loginStatus:"{loginStatus}",registeredStatus:"{registeredStatus}"},metricsCallData:{dl:["fd-results-load"]},events:["findDealerTool"],callOptions:{asPage:true},variableOptions:{variables:[{excludeFromInitialState:true,fireOnce:true,variableName:"tool"},{excludeFromInitialState:true,fireOnce:true,variableName:"event"}]}}};
metricsConfig.variables={cksVisitID:{dl:[{user:{cksVisitID:""}}]},variantName:{dl:[{page:{variantName:""}}]},contentNameID:{dl:[{onclick:{contentNameID:""}}]},contentSubType:{dl:[{onclick:{contentSubType:""}}]},contentType:{dl:[{onclick:{contentType:""}}]},hierarchy:{dl:[{page:{hierarchy:""}}]},leadOptin:{dl:[{user:{leadOptin:""}}]},leadType:{dl:[{user:{leadType:""}}]},loginStatus:{dl:[{user:{loginStatus:""}}]},registeredStatus:{dl:[{user:{registeredStatus:""}}]},onclickLinkName:{dl:[{onclick:{onclickLinkName:""}}]},onclick:{dl:[{onclick:{onclick:""}}]},pageName:{dl:[{page:{pageName:""}}]},pageNameNoVehicle:{dl:[{page:{pageNameNoVehicle:""}}]},radUIVersion:{dl:[{page:{radUIVersion:""}}]},referredTo:{dl:[{onclick:{referredTo:""}}]},siteSection:{dl:[{page:{siteSection:""}}]},tool:{dl:[{page:{tool:""}}]},toolDescriptor:{dl:[{page:{toolDescriptor:""}}]},userLanguage:{dl:[{page:{userLanguage:""}}]},"{make}":{val:function(){var a=jQuery("#metaMake").attr("data-make");
return a?a.toLocaleLowerCase():""
}},"{nameplate}":{val:function(){return""
}},"{model}":{val:function(){return""
}},"{nameplateCombined}":{val:function(c,b,a){return b("{make}",a,null)+" "+b("{nameplate}",a,null)+" "+b("{model}",a,null)
}},"{sitePrefix}":{val:function(){if(metrics.isUS()){return"fv"
}else{if(metrics.isCA()){return"foc"
}}}},"{userLanguage}":{val:function(){var a="eng";
if(window.MP&&window.MP.UrlLang){switch(window.MP.UrlLang){case"en":a="eng";
break;
case"es":a="esp";
break;
case"fr":a="fre";
...
}},"{loginStatus}":{val:function(){var a="logged out";
var b=metrics.getCookieSub("cdrProfile","cdrprofileprovider",null,"|");
if(b){if(b==="GOOGLE"){a="login:google"
}else{if(b==="FACEBOOK"){a="login:facebook"
}else{if(b==="YAHOO"){a="login:yahoo"
}else{if(b==="OWNER"){a="login:ford account"
}else{if(b==="ANONYMOUS"){a="login:current pc"
}}}}}}return a
}},"{radUIVersion}":{val:function(){var a=jQuery(window).width();
if(a<768){return"ui:rad:mobile"
}if(a>=768&&a<992){return"ui:rad:tablet"
}if(a>=992){return"ui:rad:pc"
}}},"{cksVisitID}":{val:function(){return""
}},"{contentModulePageName}":{val:function(){var a=jQuery("#metaAnalytics").attr("data-analytics-contentModulePageName");
return a
}},"{contentModuleTemplate}":{val:function(){return"none"
}},"{registeredStatus}":{val:function(){return""
}}};
metricsConfig.events={findDealerBrandReferral:{dl:[{event:{action:"find dealer | brand referral"}}]},findDealerTool:{dl:[{event:{action:"find dealer|tool"}}]},clickToCall:{dl:[{event:{action:"click to call"}}]},clickToCallDealer:{dl:[{event:{action:"click to call dealer|tool"}}]},clickToCallSales:{dl:[{event:{action:"tool"}}]},tool:{dl:[{event:{action:"tool"}}]},brandReferral:{dl:[{event:{action:"brand referral"}}]}};
metrics.init(metricsConfig);
(function(){if(window.metrics){window.appendCampaignCodesTarget=function(a,d){var e=metrics.getCookieSub("FPI","model")||"na";
var f=jQuery("#metaMobile").attr("data-mobile");
if(f){var b=metrics.getCookie("fv_v16");
if(b){var h=b.split("|");
if(h&&h.length>1){b=h[1];
if(b){e=b
}}}}var g=jQuery("#metaMake").attr("data-make");
if(e&&(e!=="na")&&g&&e.toLowerCase().indexOf(g.toLowerCase())===-1){e=g.toLowerCase()+" "+e
}var c=[{componentSelector:"billboardHomepage",slideSelector:"li.slide",componentName:"billboard"},{componentSelector:"incentivesAndOffers",slideSelector:"li.slide",componentName:"incentives"},{componentSelector:"smartNextSteps",slideSelector:"li.slide",componentName:"smartnextsteps"},{componentSelector:"brandGallery",componentName:"brandgallery"},{componentSelector:"showroomBillboard",componentName:"showroom"},{componentSelector:"locateDealerTrigger",dynamicTokens:true,componentName:"finddealer"}];
if(a&&d){var i=jQuery.map(c,function(k,j){if(k.componentSelector===d){return k
}});
if(i&&i.length>0){i=i[0];
if(i&&i.slideSelector&&jQuery(a).first().find(i.slideSelector).length>0){jQuery(a).first().find(i.slideSelector).each(function(k,l){var j="slide"+k.toString();
jQuery(this).find("a").each(function(m,p){var o="";
if(window.ttMETA&&Array.isArray(window.ttMETA)&&window.ttMETA.length>0){if(window.ttMETA[0]&&window.ttMETA[0].experience){o=window.ttMETA[0].experience
}}if(window.console&&window.console.log){console.log("campaign codes > "+d+" experienceName > "+o)
}if(o){var n=o+":"+i.componentName+":"+j+":"+e;
n=n.toLowerCase();
metrics.appendCampaignCode.call(this,"hptid",n)
}})
})
}else{if(i&&jQuery(a).first().length>0){jQuery(a).first().find("a").each(function(j,k){var m="";
if(window.ttMETA&&Array.isArray(window.ttMETA)&&window.ttMETA.length>0){if(window.ttMETA[0]&&window.ttMETA[0].experience){m=window.ttMETA[0].experience
}}if(window.console&&window.console.log){console.log("campaign codes > "+d+"experienceName > "+m)
}if(m){var l=m+":"+i.componentName+":na:"+e;
l=l.toLowerCase();
metrics.appendCampaignCode.call(this,"hptid",l)
}})
}}}}};
window.appendCampaignCodesHomepageTarget=function(){if(window.metrics&&window.metrics.isUS()){var a=jQuery("div[id^='component']").map(function(){if(this.id){return jQuery.trim(this.id)
}}).get();
if(a&&a.length>0){for(var b=0;
b<a.length;
b++){var c=a[b];
var e="div[id='"+c+"']";
if(jQuery(e).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var d=jQuery(e).find("meta[data-component]").first().attr("data-component");
if(d){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component key")
}if(window.appendCampaignCodesTarget){window.appendCampaignCodesTarget("#"+c,d)
}}}}}}};
if(metrics.isUS()){window.component01_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component01_onload")
}var c="component01";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
if(e){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component key")
}if(window.appendCampaignCodesTarget){window.appendCampaignCodesTarget("#"+c,e)
}}}if(jQuery(f).find(".ford-carousel").first().length>0){if(!jQuery(f).find(".ford-carousel").first().data("ford-carousel")){var b=null;
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component01 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component01 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component01 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component01 > interval activated")
}b=setInterval(d,50)
}}}};
window.component02_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component02_onload")
}var c="component02";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
...
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component02 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component02 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component02 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component02 > interval activated")
}b=setInterval(d,50)
}}}};
window.component03_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component03_onload")
}var c="component03";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
...
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component03 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component03 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component03 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component03 > interval activated")
}b=setInterval(d,50)
}}}};
window.component04_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component04_onload")
}var c="component04";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
...
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component04 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component04 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component04 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component04 > interval activated")
}b=setInterval(d,50)
}}}};
window.component05_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component05_onload")
}var c="component05";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
...
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component05 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component05 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component05 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component05 > interval activated")
}b=setInterval(d,50)
}}}};
window.component06_onload=function(a){if(window.console&&window.console.log){console.log("campaign codes > callback > component06_onload")
}var c="component06";
var f="div[id='"+c+"']";
if(jQuery(f).find("meta[data-component]").first().length>0){if(window.console&&window.console.log){console.log("campaign codes > "+c+" > found component div")
}var e=jQuery(f).find("meta[data-component]").first().attr("data-component");
...
var d=function(){if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
clearInterval(b);
if(window.console&&window.console.log){window.console.log("component06 > carousel activated : interval cleared")
}}else{if(jQuery(f).find(".ford-carousel").first().data("ford-carousel")){clearInterval(b);
if(window.console&&window.console.log){window.console.log("component06 > interval cleared")
}}}};
if(jQuery.fn.fordCarousel&&(typeof jQuery.fn.fordCarousel==="function")&&(!jQuery(f).find(".ford-carousel").first().data("ford-carousel"))){jQuery(f).find(".ford-carousel").first().fordCarousel();
if(window.console&&window.console.log){window.console.log("component06 > carousel activated")
}}else{if(window.console&&window.console.log){window.console.log("component06 > interval activated")
}b=setInterval(d,50)
}}}}
}}})();